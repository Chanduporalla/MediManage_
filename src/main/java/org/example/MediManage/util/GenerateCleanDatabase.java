...
public class GenerateCleanDatabase {
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(jdbcUrl);
            ...
            stmt.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_message_templates_key ON message_templates(template_key)");

            // Seed Default Templates
            stmt.execute("INSERT OR IGNORE INTO message_templates (template_key, subject, body_template) VALUES " +
                    "('whatsapp_invoice', NULL, '*{{pharmacy_name}}*\\n\\nHello {{customer_name}},\\n\\nYour invoice #{{bill_id}} for Rs. {{total_amount}} is attached as a PDF.\\n\\n{{care_note}}\\nThank you for choosing {{pharmacy_name}}.'), " +
                    "('email_invoice_subject', 'Your Invoice #{{bill_id}} from {{pharmacy_name}}', 'Your Invoice #{{bill_id}} from {{pharmacy_name}}'), " +
                    "('email_invoice_body', NULL, 'Dear {{customer_name}},\\n\\nPlease find attached your invoice #{{bill_id}} from {{pharmacy_name}} for Rs. {{total_amount}}.\\n\\n{{care_note}}\\n\\nRegards,\\n{{pharmacy_name}}')");
